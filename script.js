...
let stars = [];
for(let i = 0; i < 80; i++) {
    stars.push({ x: Math.random() * window.innerWidth, y: Math.random() * window.innerHeight, size: Math.random() * 1.5 + 0.5, opacity: Math.random() });
}
...
function drawNextPiece() {
    ...
    shape.forEach((row, y) => {
        row.forEach((value, x) => {
            if (value) nextPieceCtx.fillRect(offsetX + x * size, offsetY + y * size, size - 1, size - 1);
        });
    });
}
...
function drawSpaceBackground() {
    ...
    ctx.restore();
    const barWidth = 120; const barHeight = 8; const barX = centerX - barWidth / 2; const barY = meteorY - mSize - 20;
    ctx.fillStyle = 'rgba(10, 14, 39, 0.8)'; ctx.fillRect(barX, barY, barWidth, barHeight);
    ctx.fillStyle = theme.grid; ctx.fillRect(barX, barY, (meteorHp / meteorMaxHp) * barWidth, barHeight);
}
